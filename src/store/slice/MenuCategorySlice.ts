...
import {
  NewMenuCategoryParam,
  UpdateMenCategoryParam,
  deleteMenuCategoryParam,
} from "@/types/menuCategory";
import { MenuCategory } from "@prisma/client";
import { PayloadAction, createAsyncThunk, createSlice } from "@reduxjs/toolkit";
import {
  addDisableMenuCategory,
  setDisableMenuCategory,
} from "./DisableMenuCategorySlice";
...
export const updateMenuCategory = createAsyncThunk(
  ...
  async (updateMenu: UpdateMenCategoryParam, thunkApi) => {
    ...
    disableMenuCategories &&
      thunkApi.dispatch(setDisableMenuCategory(disableMenuCategories));
    onSuccess && onSuccess();
    return menuCategory;
  }
);
...
export const deleteMenuCategory = createAsyncThunk(
  ...
  async (deleteMenuCategory: deleteMenuCategoryParam, thunkApi) => {
    ...
    const response = await fetch(
      `${config.backofficeUrl}/menu-category?id=${payload.id}`,
      {
        method: "DELETE",
      }
    );
...
export const menuCategorySlice = createSlice({
  ...
  reducers: {
    ...
    replaceCategry(state, action: PayloadAction<MenuCategory>) {
      state.menusCategory = state.menusCategory.map((c) =>
        c.id === action.payload.id ? action.payload : c
      );
    },
    removeMenuCategory: (state, action: PayloadAction<number>) => {
      state.menusCategory = state.menusCategory.filter(
        (menusCategory) => menusCategory.id != action.payload
      );
    },
  },
